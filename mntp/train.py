...
def build_everything(args: arg_util.Args):
    ...
    #     # 🧩 兼容旧结构：仅修正 blocks.*.ada_lin.1. → blocks.*.ada_lin.fc.
    #     remapped_state_dict = {}
    #     num_fixed = 0
    #     for k, v in var_state_dict.items():
    #         # 仅修正 blocks 内的 ada_lin，不动 head_nm
    #         if ".ada_lin.1." in k and k.startswith("blocks."):
    #             new_k = k.replace(".ada_lin.1.", ".ada_lin.fc.")
    ...
    #     if num_fixed > 0:
    #         print(f"[Remap] fixed {num_fixed} ada_lin keys in blocks (head_nm untouched).")

    #     missing_keys = var_wo_ddp.load_state_dict(remapped_state_dict, strict=False)
...
def main_training():
    ...
    for ep in range(start_ep, args.ep):
        ...
        is_val_and_also_saving = (ep + 1) % 5 == 0 or (ep + 1) == args.ep or ep == 0
        if is_val_and_also_saving:
            ...
            print(f' [*] [ep{ep}]  (val {tot})  val_loss_mean: {val_loss_mean:.4f}, val_loss_tail: {val_loss_tail:.4f}, VAcc m&t: {val_acc_mean:.2f} {val_acc_tail:.2f},  Val cost: {cost:.2f}s')
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 50, header):
